...
//           `${apiUrl}/worksectors/szp5jxa7zni6obhy92jsga55?populate=details&populate=services.heading&populate=services.image`
//         );
...
const SkyItalia = () => {
  ...
  const heroData = details[0];
  const heroDesc = details[1];
  const challengeHeading= details[2];
  const challengePara= details[9];
  const theSolution = details[5];
  const theSolutionText = details[6];
  const bottomLine = details[7];
  const bottomLinePara = details[8];
  ...
  return (
    <div>
      ...
      <div className="relative bg-black">
        ...
        <div className="max-w-[1280px] relative z-10 py-[100px] px-[20px] md:px-auto mx-auto">
          <h1 className="text-center font-raleway text-white font-normal text-xl md:text-[40px] my-[48px]">
            <span className="text-white/[0.64]">{challengeHeading ? challengeHeading.light_heading : "Loading..."}</span> 
          </h1>
          <div className="flex flex-col gap-[48px] font-outfit font-light text-base md:text-2xl text-white/[0.64] text-center mb-[50px]">
            <p>
            {challengePara ? challengePara.text : "Loading..."}
            </p>
            <p>
            {challengePara ? challengePara.colored_text	 : "Loading..."}
            </p>
          </div>
          <div className="flex flex-col gap-[50px]">
        {details
  ...
  .map((item, index) => (
    ...
    >
        ...
        <>
          ...
          {services.map((service, index) => (
             ...
             className="solution-shadow w-[302px] h-[475px] border-2 border-white/64 bg-black/24 backdrop-blur-[200] py-[24px] px-[16px] rounded-[20px]">
              <img src={service.image.url} className="rounded-[12px] mx-auto" />
              <p className="mt-[24px] mb-[16px] font-outfit text-center font-medium text-xl md:text-2xl bg-gradient-to-r from-[#FF9966] to-white bg-clip-text text-transparent">
          ...
          <div>
            ...
            {facts.map((fact, index) => (
                <div key={index} className="text-white font-outfit text-xl">
      ...
      {fact.factDetail}
          ...
          <div className="mt-16 text-center flex flex-col md:gap-[64px] gap-[40px]">
            <Heading1 headingGray={bottomLine ? bottomLine.light_heading : "Loading..."} headingWhite={bottomLine ? bottomLine.dark_heading : "Loading..."} />
            <Text>
            {bottomLinePara ? bottomLinePara.text : "Loading..."}{" "}
              <GradientSpan>
              {bottomLinePara ? bottomLinePara.colored_text : "Loading..."}
              </GradientSpan>
            </Text>
          </div>
          <ISOSection />
