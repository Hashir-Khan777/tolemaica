      ...
      <div className="slider-container w-[100vw] flex flex-col gap-5">
        <Slider {...settings} >
          {[image1, image2, image3, image4, image1, image2, image3, image4].map((img, index) => (
            <div key={index} className='outline-0 flex flex-row justify-center items-center mr-[4px]'>
              <img src={img} alt={`Image ${index + 1}`} className='w-full h-full max-w-[200px] max-h-[200px]' />
            </div>
          ))}
        </Slider>
      </div>
