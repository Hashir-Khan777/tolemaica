...
const ServiceSlider1 = ({ cards }) => {
    ...
    useEffect(() => {
        ...
        fetch(`${apiUrl}/homeservice?populate=services.bg_image`)
            ...
            .then((data) => {
                // Map and transform the fetched data
                const services = data?.data?.services || [];
                setSliderData(services); // Update the state with the fetched data
                setHeading(data.heading)
            })
            .catch((error) => console.error("Failed to fetch services data:", error));

        // Initialize swiper after it is set
        if (swiperInstance) {
            swiperInstance.navigation.init();
            swiperInstance.navigation.update();
        }
    ...
    return (
        <div className="container rounded-lg mx-auto">
            {/* Debugging: Check if sliderData has data */}
            {sliderData.length === 0 && (
                <p className="text-center text-red-500">No cards available!</p>
            )}
            ...
            >
                {sliderData.map((card, index) => (
                    <SwiperSlide key={index}>
                        {/* Check for special cases like "TAILORED SERVICES" */}
                        {card.service === "TAILORED SERVICES" ? (
                            <div className="h-[400px] flex rounded-2xl overflow-hidden border-2 border-white">
                                <div
                                    className="relative flex flex-col justify-center py-[24px] items-center gap-6 p-5 w-full h-full bg-cover bg-center bg-no-repeat"
                                    style={{
                                ...
                                >
                                    <div className="absolute inset-0 bg-gradient-to-b from-transparent to-black/50"></div>
                                    <p className="z-10 text-2xl font-bold tracking-wide text-white text-center font-outfit uppercase">
                                        <span className="bg-gradient-to-r from-[#FF9966] to-white bg-clip-text text-transparent">
                                            Tailored
                                        </span>
                                        <span className="bg-gradient-to-r from-[#FF9966] to-white bg-clip-text text-transparent">
                            ...
                            <GridCard
                                title={card.service} // Using dynamic data from API
                                bgImage={card.bg_image?.url} // Using the fetched bg image
                                path="/"
                            />
                        )}
                    </SwiperSlide>
